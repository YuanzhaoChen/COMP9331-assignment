...
public class Server extends Thread {
    ...
    public static void main(String[] args) throws Exception {
        ...
        while (true) {
            ...
            if (in.readLine().equals("1")) { // type 1 socket is the one interacts with the forum
                ...
                Server s = new Server(newSocket); // once client connects to server, the rest of the jobs are done by
                                                  // thread
                syncLock.lock(); // common resource should not be changed by more than one thread at the same
                                 // time
                activeClientNum += 1;
                syncLock.unlock();
                s.start();
    ...
    public void run() {
        try {
            ...
            while (!operationsComplete) {
                ...
                // operationsComplete = commandsHadler(userInfo, new BufferedReader(new
                // InputStreamReader(this.connectionSocket.getInputStream())), new
                // DataOutputStream(this.connectionSocket.getOutputStream()),this.connectionSocket);
            }
    ...
    private static boolean authenticationHandler(UserInformation userInfo, BufferedReader inFromClient,
        ...
        try {
            ...
            if (isNewUser) {
                ...
                if (loggedInUsersSet.contains(userInfo.userName)) {
                    ...
                    if (loggedInUsersSet.contains(userInfo.userName)) { // double check in case user logged in at
                                                                        // another terminal during password typing
    ...
    private static boolean commandsHadler(UserInformation userInfo, BufferedReader inFromClient,
        ...
        try {
            ...
            } else if (command.equals("DWN") && operation.length == 3) {

                DWN_handler(operation, userInfo, inFromClient, outToClient, connectionSocket);
        ...
        } catch (Exception e) { // exception will throw if another client close all sockets when shutting down
                                // the server, so readLine() crashes
    ...
    // if the file is a thread file then it must be .txt in the current working
    // directory and not credentials.txt
    public static boolean fileIsThread(String fileName) {
        return fileName.length() > 4 && fileName.substring(fileName.length() - 4, fileName.length()).equals(".txt")
                && !fileName.equals("credentials.txt");
    }
    ...
    public static void initCommandsSet() {
        commandsSet.addAll(Arrays
                .asList(new String[] { "CRT", "LST", "MSP", "DLT", "RDT", "EDT", "UPD", "DWN", "RMW", "XIT", "SHT" }));
    }
        ...
        for (String fileName : fileNames) {
            if (fileIsThread(fileName)) {
                try {
                    ...
                    // first line of the thread file is the threadCreator, we can assume that it
                    // always exists
    ...
    public static void MSG_handler(String[] operation, UserInformation userInfo, DataOutputStream outToClient) {
        try {
            ...
            if (activeThreadsMap.containsKey(threadTitle)) {
                ...
                outToClient.writeBytes("Message posted to " + threadTitle + " thread\n");
            } else {
    ...
    public static void DLT_handler(String[] operation, UserInformation userInfo, DataOutputStream outToClient) {
        try {
            ...
            if (activeThreadsMap.containsKey(threadTitle)) { // check whether thread exist
                ...
                if (messageNumber > 0 && messageNumber <= targetThread.totalMessageNum()) { // check whether
                                                                                            // messageNumber is valid
                    if (userInfo.userName.equals(targetThread.getAuthorAtLine(messageNumber))) { // check whether user
                                                                                                 // has the right to
                                                                                                 // delete
    ...
    public static boolean SHT_handler(String[] operation, DataOutputStream outToClient) {
        try {
            if (operation[1].equals(adminPassword)) { // check password correct
                // before closing all sockets, we need to delete all active threads and
                // associated files
                Iterator<String> itr = activeThreadsMap.keySet().iterator();
                while (itr.hasNext()) {
    ...
    // since when this command is called the command handler must finish, so always
    // return true
    ...
    public static void UPD_handler(String[] operation, UserInformation userInfo, BufferedReader inFromClient,
            DataOutputStream outToClient, Socket connectionSocket) {
        syncLock.lock();
        try {
            String threadTitle = operation[1];
            ...
            if (activeThreadsMap.containsKey(threadTitle)) {
                String canonicalFileName = threadTitle + "-" + fileName; // this is the real name store on the server
                                                                         // side
    ...
    public static void DWN_handler(String[] operation, UserInformation userInfo, BufferedReader inFromClient,
            DataOutputStream outToClient, Socket connectionSocket) {
        syncLock.lock();
        try {
            String threadTitle = operation[1];
            String fileName = operation[2];
            String canonicalFileName = threadTitle + "-" + fileName;
            File file = new File(canonicalFileName);
            if (activeThreadsMap.containsKey(threadTitle)) {
                if (file.exists()) {
                    outToClient.writeBytes("OK\n"); // tell the cliet server is ready to send file
                    ...
                    while ((count = f.read(b)) > 0) {
                        outToClient.write(b, 0, count);
                        outToClient.flush();
                    }
                    f.close();
                    System.out.println("File download successful");
                    //outToClient.writeBytes("File download successful\n");
                } else {
                    outToClient.writeBytes("Not OK\n");
                    System.out.println("Requested file not found");
                    outToClient.writeBytes("No file found\n");
                }
            } else {
                outToClient.writeBytes("Not OK\n");
                System.out.println("Thread " + threadTitle + " does not exist");
            ...
            System.out.println("DWN_handler crashes");
        }
        syncLock.unlock();
    }
